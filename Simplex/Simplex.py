...
def find_entering():
    """ This method looks up the entering variable using Bland's Rule: selects the variable with the least index if more possible entering variables exist. """
    actual = None
    for i in range(1,len(z)):
        if z[i] > 0 and (non_basic[i-1] < actual or actual == None):
...
def find_leaving(e_idx):
    ...
    for i in range(0,len(A_matrix)):
        if A_matrix[i][e_idx] < 0 and (-1*b[i]/A_matrix[i][e_idx] <= bound or bound == None):
            if actual == None:
                actual = basic[i]
...
def write_output(objective_value, pivoting_steps):
    """ This method writes the output to a file with the name of the input file -- adding a '.sol' at the end of the file name. """
...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        ...
        print 'This test requires an input file.  Please select one from the data directory. (i.e. python Simplex.py ./data/part1.dict)'
